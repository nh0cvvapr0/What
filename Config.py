...
os.chdir("E:/pythonProject/pythonProject")
...
class SettingsApp:
    def __init__(self, master):
        ...
        self.dynamic_label = tk.Label(self.tabControl, text="sss")
        self.dynamic_label.pack(padx=10, pady=10)
        self.load_dynamic_label()
    ...
    def load_dynamic_label(self):
        threading.Thread(target=self.update_label).start()

    def update_label(self):
        while self.running:  # Check the flag
            self.set_label_text ("No Current Selection.")
            time.sleep(1)
            self.set_label_text("Select to edit.")
            time.sleep(1)
    ...
    def on_close(self):
        ...
        # Create a Toplevel window to display the message
        top = tk.Toplevel(self.master)
        top.title("Info")
        # Create a Label widget to display the message
        time.sleep(1)
        label = tk.Label(top, text="Closing App." + "\n GOOD BYE!!!")
        label.pack(padx=20, pady=20)
        # Close the window after 1 second
        top.after(1000, top.destroy)

        # Close the main window after 1 second
        self.master.after(1000, self.master.destroy)

    def set_label_text(self, text):
        self.dynamic_label.config(text=text)
    ...
    def check_selection(self):
        ...
        if hasattr(self, 'task_listbox') and self.task_listbox.winfo_exists():
            ...
            if task_selection:
                self.set_label_text(self.task_listbox.get(task_selection))
        ...
        if hasattr(self, 'staff_listbox') and self.staff_listbox.winfo_exists():
            ...
            if staff_selection:
                self.set_label_text(self.staff_listbox.get(staff_selection))
        ...
        if hasattr(self, 'assignor_listbox') and self.assignor_listbox.winfo_exists():
            ...
            if assignor_selection:
                self.set_label_text(self.assignor_listbox.get(assignor_selection))

        # Schedule the next check after 500 ms (0.5 second)
        self.master.after(50, self.check_selection)
